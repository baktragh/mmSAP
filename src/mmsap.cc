/* mmSAP v3.2.6
   ...
   Copyright (C) 2011-2014 Michael Kalous <zylon@post.cz>
...
#define MMSAP_VERSION_STRING "mmSAP-3.2.6"

/*Prototypes*/
void loadAndPlayFile(Glib::ustring fspec,bool shout=true);
void onRemoteRun(GApplication* app,GApplicationCommandLine* c, gpointer udata);
...
int main(int argc, char** argv) {
    ...
    /*Create application*/
    app = g_application_new("cz.baktra.mmsap",G_APPLICATION_HANDLES_COMMAND_LINE);
    if (app==NULL) return -1;
    
    /*Register the command line callback for application*/
    g_signal_connect (app, "command-line", G_CALLBACK (onRemoteRun), NULL);
    g_application_register(app,NULL,NULL);

    /*Check whether application is remote. If it is, just pass command line*/
    ...
    gui->create(preferences);
    gui->connectSignals();
    
    /*Update GUI according to preferences*/
    gui->updatePreferencesChanged(preferences,true);
    /*Playlist Tree View setup*/
    gui->setupPlaylistView(&(playlist->getColumns()->clmCurrent),&(playlist->getColumns()->clmMarkup),playlist->getModel());
    /*Update version*/
...
void onRemoteRun(GApplication* app,GApplicationCommandLine* c, gpointer udata) {
    ...
    args=g_application_command_line_get_arguments(c,&count);

    /*If number of arguments is not sufficient, just bail out*/
    if (count<2 || args==NULL || args[1]==NULL) return;

    /*Play a specified file*/
    Glib::ustring fspec(args[1]);

    #ifdef DEBUG_PRINTOUTS
    printf("Remote command line: %s\n",args[1]);
    #endif

    loadAndPlayFile(Glib::ustring(fspec),false);

    /*Free passed command line*/
    g_strfreev(args);
}
...
void on_handleIgnoreTimeTag() {
    if (gui->mniIgnoreTimeTag->get_active()==true) {
        preferences->setIgnoreTimeTag(true);
    }
    else {
        preferences->setIgnoreTimeTag(false);
    }
}
/*About ***********************************************************************/
...
void on_addFileToPlaylist() {
    ...
    gui->fcdAddToPlaylist->set_current_folder(gui->fcdAddToPlaylist->get_current_folder()); /*Emulate oldschool behavior*/
    int r=gui->fcdAddToPlaylist->run();
    gui->fcdAddToPlaylist->hide();
    
    /*If something selected, add to the playlist*/
    if (r==1) {
        bool b=playlist->addFile(gui->fcdAddToPlaylist->get_filename());
        if (b==false) {
            gui->runMessageDialog("Error","Error when adding file","File could not be added to the playlist");
        }
    }
}
...
void on_addDirectoryToPlaylist() {
    ...
    if (r==1) {
        ...
        if (b==false) {
            gui->runMessageDialog("Error","Error when adding directory","Directory could not be added to the playlist");
        }
    }
}
...
void loadAndPlayFile(Glib::ustring fspec,bool shout) {
    ...
    if (et.length()>0) {
        ...
        gui->lblFilename->set_tooltip_text(Glib::ustring(g_strstrip((gchar*)sti.c_str())));
    }
...
void on_fcdBrowseAndPlayFileActivated() {
    ...
    if (playlist->isFile(pickedFileRef->get_path())) {
        ...
        playlist->clearCurrentColumn();
        gui->recentManager->add_item(pickedFileRef->get_uri());
        //gui->fcdBrowseAndPlay->set_current_folder(pickedFileRef->get_parent()->get_path());
    }
}
