...
SDLPlayer::SDLPlayer(Preferences* pf, GUI* gi) {
...
int SDLPlayer::load(Glib::ustring filespec) {
    ...
    /*Start playing*/
    if (playOrPause(true) < 0) return PLAYER_LP_SDL_ERROR;

    return PLAYER_LP_OK;

}

int SDLPlayer::playOrPause(bool afterLoad) {
...
void SDLPlayer::first() {
    if (playerState == PLAYER_EMPTY) return;
    currentSong = 0;
    changeSubsong();
}

void SDLPlayer::previous() {
...
void SDLPlayer::seek(int tgtMiliseconds) {
...
Glib::ustring SDLPlayer::getLastError() {
    return errorMessage;
}

int SDLPlayer::closeSoundCard() {
#ifdef DEBUG_PRINTOUTS
...
void playLoopNormal(void *udata, Uint8 *stream, int len) {
    ...
    SDLPlayer* plr = (SDLPlayer*) udata;
    /*Seek request ?*/
    if (plr->seekToMiliseconds >= 0) {
        plr->seek(plr->seekToMiliseconds);
    }

    generatedBytes = ASAP_Generate((plr->state), stream, len, ASAPSampleFormat_S16_L_E);

    position = (int) ASAP_GetPosition(plr->state);

    if (abs(position - plr->lastPosition) > 1000) {
        Glib::signal_idle().connect(sigc::mem_fun(*plr, &SDLPlayer::tick), false);
        plr->lastPosition = position;
    }

    if (generatedBytes != len && plr->playerState == PLAYER_PLAYING) {
        plr->cancelRepeatFlag = false;
        Glib::signal_idle().connect(sigc::mem_fun(*plr, &SDLPlayer::handleRepeat), false);
    }
...
int SDLPlayer::openSoundCard(int channels) {

#ifdef DEBUG_PRINTOUTS
    printf("openSoundCard: channels: %d\n", channels);
#endif
...
Glib::ustring SDLPlayer::getFilename(Glib::ustring fspec) {
...
bool SDLPlayer::visualizationTick(int ti) {
...
/*Seek request*/
void SDLPlayer::setSeekRequest(double d) {
    int tgt = (int) d;
    if (d < 0 || getSubsongLength(currentSong) < 1) seekToMiliseconds = -1;
    else seekToMiliseconds = tgt;
}

void SDLPlayer::setChannelsMute(int mute) {
    ASAP_MutePokeyChannels(state, mute);
    muteMask = mute;
}

bool SDLPlayer::isEmpty() {
    if (playerState == PLAYER_EMPTY) return true;
    return false;
}

Glib::ustring SDLPlayer::getCurrentFilespec() {
    return currentFilespec;
}

bool SDLPlayer::allocateBuffer() {

#ifdef DEBUG_PRINTOUTS
    printf("SDLPlayer::allocateBuffer() %d\n", bufferSize);
#endif
...
int SDLPlayer::normalizeSAP(unsigned char** _buf, int buflen) {

    unsigned char* alternateBuffer = NULL;
    unsigned char* buf = NULL;

#ifdef DEBUG_PRINTOUTS
    printf("SDLPlayer::normalizeSAP()\n");
#endif
